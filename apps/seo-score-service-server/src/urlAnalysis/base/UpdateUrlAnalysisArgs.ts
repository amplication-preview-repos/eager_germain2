/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UrlAnalysisWhereUniqueInput } from "./UrlAnalysisWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { UrlAnalysisUpdateInput } from "./UrlAnalysisUpdateInput";

@ArgsType()
class UpdateUrlAnalysisArgs {
  @ApiProperty({
    required: true,
    type: () => UrlAnalysisWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => UrlAnalysisWhereUniqueInput)
  @Field(() => UrlAnalysisWhereUniqueInput, { nullable: false })
  where!: UrlAnalysisWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => UrlAnalysisUpdateInput,
  })
  @ValidateNested()
  @Type(() => UrlAnalysisUpdateInput)
  @Field(() => UrlAnalysisUpdateInput, { nullable: false })
  data!: UrlAnalysisUpdateInput;
}

export { UpdateUrlAnalysisArgs as UpdateUrlAnalysisArgs };
